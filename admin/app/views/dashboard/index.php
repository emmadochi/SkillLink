    ...
    <div class="stat-card">
        <div class="stat-header">
            <div class="stat-info">
                ...
                <div class="value" id="stat-total-users"><?php echo number_format($total_users); ?></div>
            </div>
    ...
    <div class="stat-card">
        <div class="stat-header">
            <div class="stat-info">
                ...
                <div class="value" id="stat-active-artisans"><?php echo number_format($active_artisans); ?></div>
            </div>
    ...
    <div class="stat-card">
        <div class="stat-header">
            <div class="stat-info">
                ...
                <div class="value" id="stat-daily-bookings"><?php echo $recent_bookings; ?></div>
            </div>
            <div class="stat-icon" style="background: var(--info-bg); color: var(--info);">
    ...
    <div class="stat-card" style="border-left: 4px solid var(--accent);">
        <div class="stat-header">
            <div class="stat-info">
                ...
                <div class="value" id="stat-revenue" style="color: var(--success);"><?php echo $revenue; ?></div>
            </div>
...
function updateDashboard() {
    fetch('<?php echo admin_url('dashboard/stats'); ?>', {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(response => response.json())
    .then(data => {
        // Update stats
        document.getElementById('stat-total-users').innerText = new Intl.NumberFormat().format(data.total_users);
        document.getElementById('stat-active-artisans').innerText = new Intl.NumberFormat().format(data.active_artisans);
        document.getElementById('stat-daily-bookings').innerText = data.recent_bookings;
        document.getElementById('stat-revenue').innerText = data.revenue;

        // Update table
        const body = document.getElementById('recent-artisans-body');
        if (data.pending_artisans.length === 0) {
            body.innerHTML = `<tr><td colspan="5" class="text-center" style="padding: 4rem;"><span class="text-muted">No pending applications.</span></td></tr>`;
            return;
        }

        let html = '';
        data.pending_artisans.forEach(a => {
            const statusClass = a.status === 'approved' ? 'badge-success' : (a.status === 'pending' ? 'badge-warning' : 'badge-danger');
            const skills = (a.skills || a.skill || 'Uncategorized');
            const avatarChar = a.name.charAt(0).toUpperCase();
            
            html += `
                <tr>
                    <td>
                        <div class="flex-row gap-3">
                            <div class="avatar-circle" style="width: 32px; height: 32px; font-size: 11px;">${avatarChar}</div>
                            <strong class="text-sm">${a.name}</strong>
                        </div>
                    </td>
                    <td><span class="text-sm">${skills}</span></td>
                    <td>
                        <div class="flex-row gap-2 text-muted text-sm">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                            ${a.location_name || 'N/A'}
                    ...
                    <td><span class="status-badge ${statusClass}">${a.status.charAt(0).toUpperCase() + a.status.slice(1)}</span></td>
                    <td style="text-align: right;">
                        <a href="<?php echo admin_url('user/details?id='); ?>${a.id}" class="btn-premium btn-primary btn-sm">Review</a>
                    </td>
                </tr>
